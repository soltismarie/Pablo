...
const App: React.FC = () => {
  ...
  const [level, setLevel] = useState<ProficiencyLevel>('B2');
  ...
  const [isTutorSpeaking, setIsTutorSpeaking] = useState<boolean>(false);
  const [tutorExpression, setTutorExpression] = useState<TutorExpression>('idle');
  ...
  const handleFetchNewExercise = useCallback(async () => {
  ...
  const playAudio = (audioBuffer: AudioBuffer, isHappyResult = false) => {
    ...
    source.onended = () => {
        ...
        if (isHappyResult) {
            ...
            setTimeout(() => setTutorExpression('idle'), 2500);
        } else {
            setTutorExpression('idle');
        }
    };
